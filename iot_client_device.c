...
//		name_msg[str_len-1] = 0;   //'\n' 제거
		...
		if(!strcmp(pArray[1],"SENSOR") && (i == 6)){
			...
			waterlevel = atoi(pArray[5]);
  			sprintf(sql_cmd, "insert into cultivation(name, date, time, illu, temp, humi, waterlevel) values('%s',now(),now(),%d,%f,%f,%d)",pArray[0],illu, temp, humi, waterlevel);
